    ...
    class ButtonMixin extends ContextOpenMixin(SuperClass) {
      ...
      `;
        ...
        renderBlock({ hasInternalTooltip, modifiers = {}, props = {} } = {}) {
            ...
            const body = x `
        <span class=${bem$c("body")}>${this.renderContent()}</span>
        <span class=${bem$c("hit-area")} ${n$3(this.onHitAreaNode)}></span>
...
const MenuItemMixin = (superClass) => {
    class MenuItemMixin extends superClass {
        ...
        <span class=${bem$8("hit-area")} ${n(this.onHitAreaNode)}></span>
      `;
        }
        /**
...
class MenuButtonItem extends MenuItemMixin(ContextOpenMixin(BaseMixin(s$8))) {
     ...
     */
    renderBlock() {
        const { active, disabled, type, href, rel, target, download, contextOpen, assistExpanded } = this;
      ...
      <button
        class=${classes}
        aria-disabled=${disabled ? "true" : A$1}
        aria-expanded=${assistExpanded || A$1}
        tabindex=${disabled ? -1 : A$1}
        @click=${this.onClick}
        type=${type || A$1}
      >
...
__decorate$9([
    n$6({ type: String, attribute: "assist-expanded" })
], MenuButtonItem.prototype, "assistExpanded", void 0);
__decorate$9([
    i$2(".menu-item")
...
class MenuItemGroup extends BaseMixin(s$8) {
    ...
    render() {
        ...
        return x `
      <div class=${bem$6({ open })}>
        <bds-menu-button-item
          id=${bem$6("label")}
          class=${bem$6("label")}
          assist-expanded=${open ? "true" : "false"}
        ...
        <div
          class=${bem$6("body")}
          aria-labelledby=${label ? bem$6("label") : A$1}
        >
          <slot></slot>
        </div>
      </div>
...
const styles$2 = [
    ...
    /**
     * This is to fix an edge-case where a popover / positioning library (e.g. Popper / Floating UI)
     * walks through the light DOM until it finds the nearest layer (e.g. position: relative),
     * in order to calculate a popovers offset position. In which case, it will bypass any layer
     * within this element's Shadow DOM.
     *
     * We apply a light DOM style to the slotted main element as a fail-safe, to circumvent any
     * issues, if the popover library's 'root' container is not scoped.
     */
    ::slotted([slot="main"]) {
      position: relative;
    }
